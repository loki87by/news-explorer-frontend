...
function Navigation(props) {
  ...
  return (
    <section className="Navigation">
      ...
      : ''}
      {props.loggedIn ?
      <button className={`Navigation__button ${props.isSavedNewsPage && (props.screenWidth > 400) && "Navigation_black"}`} onClick={props.logOut} type="reset">{props.currentUser.name} [-&gt;</button> :
      <button className={`Navigation__button ${props.isSavedNewsPage && (props.screenWidth > 400) && "Navigation_black"}`} type="button" onClick={props.handleLoginClick}>Авторизоваться</button>}
    </section>
  )
};
